...
export async function createUserWithEmailAndPasswordAndProfile(
  email,
  password,
  ...
  try {
    ...
    console.log(
      "User account and Firestore profile created successfully for:",
      user.email,
      "with username:",
      defaultUsername
    );
...
export function initAuthCheck(
  isAuthPage = false,
  authPageRedirectTo = "home.html",
  unauthenticatedRedirectTo = "auth.html"
) {
  onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
      ...
      const emailDisplayElements = document.querySelectorAll(
        ".user-email-display"
      );
      emailDisplayElements.forEach(
        (el) => (el.textContent = placeholderUsername)
      );
      ...
      try {
        ...
        if (userDocSnap.exists()) {
          ...
          console.warn(
            "No user profile found in Firestore for:",
            user.uid,
            ". Creating one now."
          );
        ...
        const moodDisplayElements = document.querySelectorAll(
          ".user-preferred-mood-display"
        );
        moodDisplayElements.forEach((el) => {
          el.textContent = displayPreferredMood;
        });
        ...
        console.error(
          "Error fetching or creating user profile in initAuthCheck:",
          error
        );
        if (loader) loader.style.display = "none";
        emailDisplayElements.forEach(
          (el) => (el.textContent = "Error loading name")
        );
        window.currentUserRoles = [];
