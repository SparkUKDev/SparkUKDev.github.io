...
import {
  ...
  getDoc,
  onSnapshot,
} from "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js"; // Added onSnapshot for real-time updates
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (user) {
      ...
      try {
        ...
        if (userDocSnap.exists()) {
          ...
          console.warn(
            "User profile not found in Firestore for current user on settings page. Creating default profile."
          );
        ...
        if (status) {
          status.textContent =
            "Error loading settings. Please contact support.";
  ...
  saveBtn &&
    saveBtn.addEventListener("click", async (e) => {
      ...
      const newDisplayName = displayNameInput
